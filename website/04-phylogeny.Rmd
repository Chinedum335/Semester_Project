#  Species-level Phylogeny of the Cystopteridaceae Ferns {#phylogeny-design}
...
```{r image4, eval = TRUE, echo = FALSE, fig.cap="MCMC trace from the Cystopteridaceae `r homologizer` analysis", fig.align='center', out.width='100%'}
...
```
Then to plot these distributions. `plotTrace()` produces a list of ggplot2 objects, with multiple plots of multiple runs in the trace object.
...
`homologizer` analysis ends with the creation of a tree file `homologizer_output/homologizer_map.tree` that can be plotted and rooted in software like [FigTree](http://tree.bio.ed.ac.uk/software/figtree/) or [APE](https://cran.r-project.org/web/packages/ape/index.html). However, these software can't be used to properly summarize the phasing estimates of  `homologizer` analysis. R can be used to achieve this task. The piece of R code below summarizes and plots the phasing estimates of  `homologizer` analysis. This code can be found [here](https://github.com/Chinedum335/Semester_Project/tree/main/5_Code)
