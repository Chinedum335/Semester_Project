...
This chapter provides a detailed assessment of the results obtained following `homologizer`analysis.
...
The code chunk for plotting trace can be found below. Let's load in all the packages that are required for all our assessments before running the code. 
...
`homologizer` run ends with the creation of a tree file `homologizer_output/homologizer_map.tree` that can be plotted and rooted in software like [FigTree](http://tree.bio.ed.ac.uk/software/figtree/) or [APE](https://cran.r-project.org/web/packages/ape/index.html). However, these software (Figtree or Ape) can't be used to properly summarize the phasing estimates of `homologizer` run in the most visualistic way. `R` can be used to achieve this task. The chunk of `R` code below summarizes and plots the phasing estimates following `homologizer` analysis. This code can be found [here](https://github.com/Chinedum335/Semester_Project/tree/main/5_Code)
